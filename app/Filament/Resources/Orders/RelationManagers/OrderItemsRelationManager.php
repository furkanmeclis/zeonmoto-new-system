        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('line_total')
                    ...
                    ->money('TRY')
                    ->sortable(),
                TextColumn::make('price_rules_snapshot')
                    ...
                    ->formatStateUsing(function ($state) {
                        ...
                        $rules = $state['applied_rules'] ?? [];
                        if (empty($rules)) {
                            return 'Kural yok';
                        }

                        return count($rules) . ' kural uygulandı';
                    })
                    ->tooltip(function ($record) {
                        if (empty($record->price_rules_snapshot)) {
                            return null;
                        }

                        $snapshot = $record->price_rules_snapshot;
                        $basePrice = $snapshot['base_price'] ?? 0;
                        $finalPrice = $snapshot['final_price'] ?? 0;
                        $rules = $snapshot['applied_rules'] ?? [];

                        $tooltip = "Temel Fiyat: ₺" . number_format($basePrice, 2) . "\n";
                        $tooltip .= "Final Fiyat: ₺" . number_format($finalPrice, 2) . "\n";
                        $tooltip .= "Uygulanan Kurallar: " . count($rules);
